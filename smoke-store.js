...
(async () => {
  ...
  const combined = await new Promise((resolve) => {
    ...
    ws.send(JSON.stringify({
      ...
      bpayRows: [{ date: "2026-08-10", reference: "VIX122334", recPayType: "Debtor Payment Receipt", amount: "150.00", bookingNo: "13157" }],
      mintRows: [{ transNo: "P.0000004123", amount: "400.00", amountCents: 40000, toCompany: "READY ROOMS" }],
  ...
  ok("holding both reports' rows", both && both.rows.length === 2, both && String(both.rows.length));
  ok("each row knows which report it came from",
    both && both.rows.map((r) => r.src).join(",") === "bpay,mint",
    both && both.rows.map((r) => r.src).join(","));
  ok("numbered across both, so no row overwrites another",
    both && both.rows.map((r) => r.n).join(",") === "1,2",
    both && both.rows.map((r) => r.n).join(","));
  ok("and its money is both reports added up", both && both.totals.amountCents === 55000,
    both && String(both.totals.amountCents));

  const dash = await get("/api/overview");
  ok("the dashboard counts it under both sources",
    dash.bySource.bpay === 2 && dash.bySource.mint === 1, JSON.stringify(dash.bySource));
