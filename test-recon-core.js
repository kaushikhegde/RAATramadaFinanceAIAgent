...
check("a missing header is refused outright",
  ...
  "the header is missing: recPayType, bookingNo");
...
{
  // The four cases, stated by the user, against two segments of 200.
  ...
  check("300 takes ONE segment, not both", one.allocation, [{ segId: "A", amount: "200.00" }]);
  check("also part allocated", one.status, "Part allocated");
  ...
  ok("and says the cheapest is still too big",
    /cheapest still owes \$200\.00/.test(none.reason), none.reason);
  ...
  // Ties go to the cheapest segments — "lowest Debtor Due first".
  const tie = C.decideAllocation(20000, [
    { segId: "a", debtorDue: "100.00" }, { segId: "b", debtorDue: "100.00" }, { segId: "c", debtorDue: "200.00" },
  ]);
  check("two hundreds beat one two-hundred",
    tie.allocation, [{ segId: "a", amount: "100.00" }, { segId: "b", amount: "100.00" }]);
